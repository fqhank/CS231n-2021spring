...
def rnn_backward(dh, cache):
    ...
    db = np.zeros((dWh.shape[0],))
    ddprev_h = 0

    for i in reversed(range(dh.shape[1])):
        ddx,ddprev_h,ddWx,ddWh,ddb = rnn_step_backward(dh[:,i,:]+ddprev_h,cache[i])
        dx[:,i,:] += ddx
        dWx +=ddWx
        dWh +=ddWh
        db += ddb
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x,prev_h,prev_c,next_c,Wx,Wh,i,f,g,o)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    pass
    x,prev_h,prev_c,next_c,Wx,Wh,i,f,g,o = cache
    H = prev_h.shape[1]
    da = np.zeros((prev_h.shape[0],4*H))
    ...
    da[:,:H], da[:,H:2*H], da[:,2*H:3*H], da[:,3*H:4*H] = di*i*(1-i), df*f*(1-f), do*o*(1-o), dg*(1-g**2)
    db = np.sum(da,axis=0)
    dx = np.dot(da,Wx.T)
    dWx = np.dot(x.T,da)
