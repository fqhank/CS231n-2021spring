...
def affine_forward(x, w, b):
    ...
    num_x = x.shape[0]
    dim_per_x = w.shape[0]
    re_x = x.reshape(num_x,dim_per_x)
    out = re_x.dot(w) + b.reshape(-1,b.shape[0])
...
def svm_loss(x, y):
    ...
    dx[margin<=0] = 0
    margin[margin<=0] = 0
    dx[range(N),y] -= np.sum(margin,axis=1)
...
def softmax_loss(x, y):
    ...
    # dx = -1 * temp / (norm_scores[range(num_train),y]).reshape((num_train,-1)) / (np.sum(exp_scores,axis=1).reshape((num_train,-1)))**2 * exp_scores
    num_train = x.shape[0]
    exp_scores = np.exp(x)
    norm_scores = exp_scores / np.sum(exp_scores,axis=1).reshape((num_train,-1))
